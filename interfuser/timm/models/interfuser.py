...
class Interfuser(nn.Module):
    def __init__(
        ...
        self.with_dvs = with_dvs
        self.dvs_input_size = dvs_input_size
        ...
        if use_different_backbone:
            ...
            if lidar_backbone_name == "r50":
                ...
                self.lidar_backbone = resnet18d(
                    pretrained=False, in_chans=2, features_only=True, out_indices=[4]
                )
        ...
        if self.with_dvs:
            if dvs_backbone_name == "r50":
                self.dvs_backbone = resnet50d(
                    pretrained=False,
                    in_chans=3,
                    features_only=True,
                    out_indices=[4],
                )
            elif dvs_backbone_name == "r26":
                self.dvs_backbone = resnet26d(
                    pretrained=False,
                    in_chans=3,
                    features_only=True,
                    out_indices=[4],
                )
            elif dvs_backbone_name == "r18":
                self.dvs_backbone = resnet18d(
                    pretrained=False,
                    in_chans=3,
                    features_only=True,
                    out_indices=[4],
            ...
            dvs_embed_layer = partial(HybridEmbed, backbone=self.dvs_backbone)
            self.dvs_patch_embed = dvs_embed_layer(
                img_size=img_size,
        ...
        # Update embed parameters to accommodate DVS views (RGB: front, left, right, center + DVS: front, left, right, center + LiDAR = 9 views)
        num_views = 9 if self.with_dvs else 5
        self.global_embed = nn.Parameter(torch.zeros(1, embed_dim, num_views))
        self.view_embed = nn.Parameter(torch.zeros(1, embed_dim, num_views, 1))
    ...
    def forward_features(
        ...
        # DVS processing (if enabled)
        if self.with_dvs:
            dvs_view_offset = 5  # DVS views start at index 5
            
            # DVS Front view processing
            if dvs_front is not None:
                dvs_front_token, dvs_front_token_global = self.dvs_patch_embed(dvs_front)
                if self.use_view_embed:
                    dvs_front_token = (
                        dvs_front_token
                        + self.view_embed[:, :, dvs_view_offset:dvs_view_offset+1, :]
                        + self.position_encoding(dvs_front_token)
                    )
                else:
                    dvs_front_token = dvs_front_token + self.position_encoding(dvs_front_token)
                dvs_front_token = dvs_front_token.flatten(2).permute(2, 0, 1)
                dvs_front_token_global = (
                    dvs_front_token_global
                    + self.view_embed[:, :, dvs_view_offset, :]
                    + self.global_embed[:, :, dvs_view_offset:dvs_view_offset+1]
                )
                dvs_front_token_global = dvs_front_token_global.permute(2, 0, 1)
                features.extend([dvs_front_token, dvs_front_token_global])
            
            # DVS Left view processing
            if dvs_left is not None:
                dvs_left_token, dvs_left_token_global = self.dvs_patch_embed(dvs_left)
                if self.use_view_embed:
                    dvs_left_token = (
                        dvs_left_token
                        + self.view_embed[:, :, dvs_view_offset+1:dvs_view_offset+2, :]
                        + self.position_encoding(dvs_left_token)
                    )
                else:
                    dvs_left_token = dvs_left_token + self.position_encoding(dvs_left_token)
                dvs_left_token = dvs_left_token.flatten(2).permute(2, 0, 1)
                dvs_left_token_global = (
                    dvs_left_token_global
                    + self.view_embed[:, :, dvs_view_offset+1, :]
                    + self.global_embed[:, :, dvs_view_offset+1:dvs_view_offset+2]
                )
                dvs_left_token_global = dvs_left_token_global.permute(2, 0, 1)
                features.extend([dvs_left_token, dvs_left_token_global])
            
            # DVS Right view processing
            if dvs_right is not None:
                dvs_right_token, dvs_right_token_global = self.dvs_patch_embed(dvs_right)
                if self.use_view_embed:
                    dvs_right_token = (
                        dvs_right_token
                        + self.view_embed[:, :, dvs_view_offset+2:dvs_view_offset+3, :]
                        + self.position_encoding(dvs_right_token)
                    )
                else:
                    dvs_right_token = dvs_right_token + self.position_encoding(dvs_right_token)
                dvs_right_token = dvs_right_token.flatten(2).permute(2, 0, 1)
                dvs_right_token_global = (
                    dvs_right_token_global
                    + self.view_embed[:, :, dvs_view_offset+2, :]
                    + self.global_embed[:, :, dvs_view_offset+2:dvs_view_offset+3]
                )
                dvs_right_token_global = dvs_right_token_global.permute(2, 0, 1)
                features.extend([dvs_right_token, dvs_right_token_global])
            
            # DVS Center view processing
            if dvs_center is not None:
                dvs_center_token, dvs_center_token_global = self.dvs_patch_embed(dvs_center)
                if self.use_view_embed:
                    dvs_center_token = (
                        dvs_center_token
                        + self.view_embed[:, :, dvs_view_offset+3:dvs_view_offset+4, :]
                        + self.position_encoding(dvs_center_token)
                    )
                else:
                    dvs_center_token = dvs_center_token + self.position_encoding(dvs_center_token)
                dvs_center_token = dvs_center_token.flatten(2).permute(2, 0, 1)
                dvs_center_token_global = (
                    dvs_center_token_global
                    + self.view_embed[:, :, dvs_view_offset+3, :]
                    + self.global_embed[:, :, dvs_view_offset+3:dvs_view_offset+4]
                )
                dvs_center_token_global = dvs_center_token_global.permute(2, 0, 1)
                features.extend([dvs_center_token, dvs_center_token_global])

        if self.with_lidar:
            # Adjust LiDAR view index based on DVS presence
            lidar_view_idx = 8 if self.with_dvs else 4
            lidar_token, lidar_token_global = self.lidar_patch_embed(lidar)
            if self.use_view_embed:
                lidar_token = (
                    lidar_token
                    + self.view_embed[:, :, lidar_view_idx:lidar_view_idx+1, :]
            ...
            lidar_token_global = (
                lidar_token_global
                + self.view_embed[:, :, lidar_view_idx, :]
                + self.global_embed[:, :, lidar_view_idx:lidar_view_idx+1]
            )
    ...
    def forward(self, x):
        ...
        # Extract DVS inputs if available
        dvs_front = x.get("dvs", None) if self.with_dvs else None
        dvs_left = x.get("dvs_left", None) if self.with_dvs else None
        dvs_right = x.get("dvs_right", None) if self.with_dvs else None
        dvs_center = x.get("dvs_center", None) if self.with_dvs else None
...
def interfuser_baseline(**kwargs):
    # Remove 'pretrained' parameter if present since Interfuser doesn't accept it
    kwargs.pop('pretrained', None)
    # Remove 'drop_rate' parameter if present since Interfuser doesn't accept it
    kwargs.pop('drop_rate', None)
    # Remove 'drop_path_rate' parameter if present since Interfuser doesn't accept it
    kwargs.pop('drop_path_rate', None)
    # Remove 'drop_connect_rate' parameter if present since Interfuser doesn't accept it
...
@register_model
def interfuser_baseline_dvs(**kwargs):
    # Extract DVS parameters from kwargs with defaults
    with_dvs = kwargs.pop('with_dvs', True)
    dvs_backbone_name = kwargs.pop('dvs_backbone_name', 'r50')
    # Remove 'pretrained' parameter if present since Interfuser doesn't accept it
    kwargs.pop('pretrained', None)
    # Remove 'drop_rate' parameter if present since Interfuser doesn't accept it
    kwargs.pop('drop_rate', None)
    ...
    model = Interfuser(
        ...
        rgb_backbone_name="r50",
        lidar_backbone_name="r18",
        dvs_backbone_name=dvs_backbone_name,
        waypoints_pred_head="gru",
        use_different_backbone=True,
        with_dvs=with_dvs,
        **kwargs
