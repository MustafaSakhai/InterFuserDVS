...
class CarlaMVDetDataset(BaseIODataset):
    def __init__(
        ...
        self.dvs_center_transform = dvs_center_transform
    ...
    def __getitem__(self, idx):
        ...
        # Load RGB data from merged rgb_full image
        ...
        if self.multi_view:
            ...
            # For single view, create dummy images to maintain compatibility
            rgb_left_image = rgb_image
            rgb_right_image = rgb_image
        ...
        if self.with_dvs:
            # Load DVS data from merged dvs_full image
            dvs_full_image = self._load_image(
                os.path.join(route_dir, "dvs_full", "%04d.png" % frame_id)
            )
            dvs_front_image = dvs_full_image.crop((0, 0, 800, 600))
            if self.multi_view:
                dvs_left_image = dvs_full_image.crop((0, 600, 800, 1200))
                dvs_right_image = dvs_full_image.crop((0, 1200, 800, 1800))
            else:
                # For single view, create dummy images to maintain compatibility
                dvs_left_image = dvs_front_image
                dvs_right_image = dvs_front_image
        ...
        if self.with_dvs:
            if self.dvs_transform is not None:
                dvs_front_transformed = self.dvs_transform(dvs_front_image)
            else:
                dvs_front_transformed = dvs_front_image
            data["dvs"] = dvs_front_transformed
            
            # Create DVS center view (same as front but potentially with different transform)
            if self.dvs_center_transform is not None:
                dvs_center_image = self.dvs_center_transform(dvs_front_image)
            else:
                dvs_center_image = dvs_front_transformed
            data["dvs_center"] = dvs_center_image
            
            if self.multi_view:
                if self.dvs_transform is not None:
                    dvs_left_image = self.dvs_transform(dvs_left_image)
                    dvs_right_image = self.dvs_transform(dvs_right_image)
                data["dvs_left"] = dvs_left_image
                data["dvs_right"] = dvs_right_image
